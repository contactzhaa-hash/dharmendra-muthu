...
export default function ContactPage() {
    ...
    return (
        <div className="bg-[#070A12] text-slate-100 min-h-screen font-sans flex flex-col selection:bg-[#00C2FF] selection:text-[#070A12]">
            <header className="border-b border-slate-800/60 bg-[#070A12]/90 backdrop-blur-md sticky top-0 z-50">
                <div className="max-w-7xl mx-auto px-8 h-24 flex items-center justify-between">
                    <Link href="/" className="font-black tracking-widest text-white text-xl uppercase font-mono">
                    ...
                    <nav className="hidden md:flex space-x-10 text-xs font-bold uppercase tracking-widest text-slate-400">
                        ...
                        <Link href="/news" className="hover:text-white transition">02 / Macro Reality</Link>
                        <Link href="/contact" className="text-[#00C2FF]">03 / Private Dispatch</Link>
                    </nav>
                    <button 
                        onClick={() => setLang(lang === 'EN' ? 'FR' : 'EN')}
                        className="px-4 py-2 border border-slate-800 bg-slate-900/50 rounded-sm text-xs font-mono text-[#00C2FF] hover:border-[#00C2FF] transition"
                    >
            ...
            <main className="flex-grow max-w-4xl mx-auto px-6 py-24 w-full flex items-center justify-center">
                <div className="p-12 bg-[#0E1322] border border-slate-800 rounded-sm flex flex-col md:flex-row items-center gap-12 w-full shadow-2xl relative">
                    ...
                    <div>
                        <span className="text-xs font-mono uppercase tracking-widest text-[#00C2FF] mb-3 block">
                            Senior Financial Infrastructure
                        </span>
                        <h1 className="text-3xl md:text-4xl font-black text-white mb-4 tracking-tight">
                            Dharmendra Aseervetha Muthu
                        </h1>
                        <p className="text-slate-300 text-sm font-light leading-relaxed mb-8">
                            Architecting robust financial stability, risk management, and long-term asset independence for high-growth operators across Canada and international markets in {currency}.
                        </p>
            ...
            {/* Elite Footer */}
            <footer className="py-12 bg-[#05070D] border-t border-slate-800/80 text-center text-xs font-mono text-slate-500 uppercase tracking-widest">
                <p>&copy; 2026 Dharmendra Aseervetha Muthu. Built for elite operators.</p>
            </footer>
        </div>
    );
}
